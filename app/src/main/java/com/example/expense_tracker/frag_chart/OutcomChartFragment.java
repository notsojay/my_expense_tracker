...
public class OutcomChartFragment extends BaseChartFragment {
    ...
    public void onResume() {
        super.onResume();
        loadData(year, month, TransactionCategory.AccountType.EXPENSE);
    }

    @Override
    protected void setAxisData(int year, int month) {
        List<IBarDataSet> sets = new ArrayList<>();
        List<MonthlyBarData> list = DBManager.getSumMoneyDailyInSelectMonth(year, month, TransactionCategory.AccountType.EXPENSE);
    ...
    protected void setYAxis(int year, int month) {
        float maxMoney = DBManager.getMaxMoneyDailyInSelectMonth(year, month, TransactionCategory.AccountType.EXPENSE);
